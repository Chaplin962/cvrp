			...
			for (j = i + 1; j < k; j++)
			{
				if (rt_nodes[j] > 1)
				{
					...
					while (curr2 != 0)
					{
						...
						{
							change1 = distD(anc1, curr2, posx, posy) + distD(curr2, suc1, posx, posy) - (distD(anc1, curr, posx, posy) + distD(curr, suc1, posx, posy));

							change2 = distD(anc2, curr, posx, posy) + distD(curr, suc2, posx, posy) - (distD(anc2, curr2, posx, posy) + distD(curr2, suc2, posx, posy));
...
__device__ float rm_diff(int *k_routes, int st, int j, float *posx, float *posy)
{
	...
	if (st == j)
	{
		if (k_routes[st] != 0)
		{
			change2 = distD(0, k_routes[st], posx, posy) - distD(0, j, posx, posy) - distD(k_routes[st], j, posx, posy);
		}
		else
	...
	else
	{
		...
		if (st1 != 0)
		{
			change2 = distD(st, k_routes[st1], posx, posy) - distD(st, j, posx, posy) - distD(k_routes[st1], j, posx, posy);
		}
		else
		{
			change2 = -(distD(st, j, posx, posy) + distD(0, j, posx, posy) - distD(0, st, posx, posy));
		}
	}
	return change2;
}
...
__device__ float add_diff(int *k_routes, int *rt_st, int j, float *posx, float *posy, int nodes, int *best_nbr, int rt_2)
{
	...
	{
		change1 = distD(rt_st[rt_2], j, posx, posy) + distD(0, j, posx, posy) - distD(0, rt_st[rt_2], posx, posy);
		*best_nbr = rt_st[rt_2];
	}
	else
	{
		...
		change1 = distD(curr_nbr, j, posx, posy) + distD(next_nbr, j, posx, posy) - distD(curr_nbr, next_nbr, posx, posy);
		*best_nbr = curr_nbr;
	}
	return change1;
}
				...
				for (i = 0; i < (rt_nodes[st] - 2); i++)
				{
					...
					for (j = i + 1; j < (rt_nodes[st] - 1); j++)
					{
						...
						for (m = j + 1; m < rt_nodes[st]; m++)
						{
							...
							change = distD(curr1, curr3, posx, posy) + distD(k_routes[curr1], k_routes[curr2], posx, posy) + distD(curr2, m_next, posx, posy) - (distD(curr1, k_routes[curr1], posx, posy) + distD(curr2, k_routes[curr2], posx, posy) + distD(curr3, m_next, posx, posy));
							lchange = change * 100;
		...
		if (rt_nodes[st] > 2)
		{
			...
			while (flag)
			{
				...
				while (curr != 0)
				{
					...
					while (j != 0)
					{
						change = 100 * (distD(i, j, posx, posy) + distD(curr, curr2, posx, posy) - (distD(i, curr, posx, posy) + distD(j, curr2, posx, posy)));
				...
				for (; stride > 0; stride--)
				{
					...
					while (count1 < (rt_nodes[x] - 1 - stride))
					{
						...
						for (count2 = count1 + stride; count2 < (rt_nodes[x] - 1); count2++)
						{
							...
							change = distD(curr2, first, posx, posy) + distD(j_next, last, posx, posy) + distD(first_prev, last_next, posx, posy) - (distD(first_prev, first, posx, posy) + distD(last, last_next, posx, posy) + distD(j_next, curr2, posx, posy));
							lchange = change * 100;
							if (lchange < minchange)
							{
								minchange = lchange;
	...
	back:
		...
		do
		{
			...
			for (y = x; y < n; y++)
			{
				if (rt_st[r] != y && !v[y])
				{
					d1 = distD(cur_i, y, posx, posy) + distD(0, y, posx, posy) - distD(cur_i, 0, posx, posy);
					if (d1 < min_d)
					{
						min_d = d1;
						min_i = y;
				...
				{
					d1 = distD(cur_i, unvst[y], posx, posy) + distD(0, unvst[y], posx, posy) - distD(cur_i, 0, posx, posy);
					if (d1 < min_d)
					{
						min_d = d1;
						min_i = unvst[y];
