			...
			for (j = rt_1 + 1; j < k; j++)
			{
				if (rt_nodes[j] > 1)
				{
					...
					while (x != 0)
					{
						...
						{
							change1 = distD(prev_id_1, x, posx, posy) + distD(x, next_id_1, posx, posy) - (distD(prev_id_1, i, posx, posy) + distD(i, next_id_1, posx, posy));

							change2 = distD(prev_id_2, i, posx, posy) + distD(i, next_id_2, posx, posy) - (distD(prev_id_2, x, posx, posy) + distD(x, next_id_2, posx, posy));
...
float add_diff(int *k_routes, int st, int j, float *posx, float *posy, int nodes, float rt_dst, int *best_nbr)
{
	...
	{
		change1 = distD(st, j, posx, posy) + distD(0, j, posx, posy) - distD(0, st, posx, posy);
		*best_nbr = st;
	}
	return change1;
}
...
__global__ void relocate_kernel(int *k_routes, int *rt_st, float *posx, float *posy, int *demands, int *cap, int Q, int n, int *rt_nodes, struct relocate_data *sd, int *rt_id, int k)
{
	...
	if (i > 0 && i < n)
	{
		...
		if (rt_nodes[rt_1] > 1)
		{
			...
			change2 = distD(next_id_1, prev_id_1, posx, posy) - distD(prev_id_1, i, posx, posy) - distD(next_id_1, i, posx, posy);
			for (j = rt_1 + 1; j < k; j++)
			{
				gap = Q - cap[j];
				if (demands[i] <= gap)
	...
	if (x > 0 && x < n)
	{
		...
		if (rt_nodes[rt_1] > 3)
		{
			...
			while (y != 0)
			{
				...
				{
					next_z = k_routes[z];
					change = 100 * (distD(x, z, posx, posy) + distD(next_x, next_y, posx, posy) + distD(y, next_z, posx, posy) - (distD(x, next_x, posx, posy) + distD(y, next_y, posx, posy) + distD(z, next_z, posx, posy)));
	...
	if (i > 0 && i < n)
	{
		...
		if (rt_nodes[rt_1] > 2)
		{
			...
			if (i == j)
			{
				...
				{
					y_n = k_routes[y];
					change1 = distD(x, y, posx, posy) + distD(x_n, y_n, posx, posy) - (distD(x, x_n, posx, posy) + distD(y, y_n, posx, posy));
					change = change1 * 100;
			...
			while (j)
			{
				if (i != j)
				{
					...
					change1 = distD(i, j, posx, posy) + distD(next_id_1, next_id_2, posx, posy) - (distD(i, next_id_1, posx, posy) + distD(j, next_id_2, posx, posy));
					change = change1 * 100;
					if (change < td[i].change)
					{
						td[i].rt = rt_1;
			...
			for (; stride > 0; stride--)
			{
				if (count1 < (rt_nodes[rt_1] - 1 - stride))
				{
					...
					for (count2 = count1 + stride; count2 < (rt_nodes[rt_1] - 1); count2++)
					{
						...
						change1 = distD(curr2, i, posx, posy) + distD(j_next, last, posx, posy) + distD(i_prev, last_next, posx, posy) - (distD(i_prev, i, posx, posy) + distD(last, last_next, posx, posy) + distD(j_next, curr2, posx, posy));
						change = change1 * 100;
