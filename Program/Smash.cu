...
// [edit] CUDA
__global__ void parallelUpdateRouteData(Route *myRoute, Params *params, LocalSearch *localsearch)
{
	int myplace = 0;
	double myload = 0.;
	double mytime = 0.;
	...
	myRoute->load = myload;
	myRoute->penalty = localsearch->penaltyExcessDuration(mytime) + localsearch->penaltyExcessLoad(myload);
	...
	myRoute->whenLastModified = localsearch->nbMoves;

	if (myRoute->nbCustomers == 0)
	{
		myRoute->polarAngleBarycenter = 1.e30;
		localsearch->emptyRoutes.insert(myRoute->cour);
	}
	else
	{
		...
		localsearch->emptyRoutes.erase(myRoute->cour);
	}
}
