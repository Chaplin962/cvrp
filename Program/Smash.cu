...
void Genetic::run()
{
	...
	for (nbIter = 0; nbIterNonProd <= params.ap.nbIter && (params.ap.timeLimit == 0 || (double)(clock() - params.startTime) < params.ap.timeLimit); nbIter++)
	{
		...
		cudaMemcpy(parallel_localSearch, &localSearch, sizeof(LocalSearch), cudaMemcpyHostToDevice);

		int numThread = params.nbClients / BLOCKS;
		run2<<<BLOCKS, numThread>>>(parallel_localSearch, offspring, params.penaltyCapacity, params.penaltyDuration);
